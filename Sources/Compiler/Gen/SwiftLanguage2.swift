...
public struct SwiftLanguage2: Language {
    ...
    class Writer {
        ...
        func file(
            ...
            for table in tables {
                declaration(for: table, isOutput: true, options: options)
            }
            
            for query in allQueries {
                modelsFor(query: query, options: options)
            }
            ...
            dbStruct(queries: queries, migrations: migrations, options: options)
                ...
                for (namespace, queries) in queries {
                    if let namespace {
                        ...
                        for query in queries {
                            declaration(for: query, databaseName: options.databaseName, options: options)
                        }
                    }
                }
            }
        ...
        private func declaration(
            ...
            if isOutput {
                extensionOn(model.name, conformance: "Otter.RowDecodable") {
                    rowDecodableInit(for: model)
                }
            }
        }
        
        private func modelsFor(
            query: GeneratedQuery,
            options: GenerationOptions
        ) {
            if case let .model(model) = query.input, !model.isTable {
                declaration(for: model, isOutput: false, options: options)
            }
            
            if case let .model(model) = query.output, !model.isTable {
                declaration(for: model, isOutput: true, options: options)
            }
        }
