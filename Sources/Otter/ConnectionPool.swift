...
public actor ConnectionPool: Sendable {
    ...
    public init(
        ...
        guard limit > 0 else {
            throw OtterError.poolCannotHaveZeroConnections
        }
    ...
    ) async throws(OtterError) -> sending Transaction {
    ...
    private func newConnection() throws(OtterError) -> SQLiteConnection {
        assert(count < limit)
        count += 1
        let connection = try SQLiteConnection(path: path)
