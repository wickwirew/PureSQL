...
struct MapTests {
    ...
    @Test func mapTransformsOutput() async throws {
        let query = Queries.Just<Void, Int>(100)
        let output = try await query.map(\.description).execute()
        #expect(output == "100")
    }
    
    @Test func mapTransformsOutput_Observation() async throws {
        let query = Queries.Just<Void, Int>(100).map(\.description)
    ...
    @Test func throwIfNotFoundReturnsDefaultError() async throws {
        ...
        await #expect(throws: OtterError.self) {
