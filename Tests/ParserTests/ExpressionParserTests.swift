...
extension ExpressionParserTests {
    func testBindParameters() {
        ...
        XCTAssertEqual(.named("variable"), try execute(parser: BindParameterParser(), source: "@variable").kind)
        XCTAssertEqual(.named("variable"), try execute(parser: BindParameterParser(), source: "$variable").kind)
        XCTAssertEqual(.named("variable::another"), try execute(parser: BindParameterParser(), source: "$variable::another").kind)
        XCTAssertEqual(.named("variable::another(suffix)"), try execute(parser: BindParameterParser(), source: "$variable::another(suffix)").kind)
    }
}
...
extension ExpressionParserTests {
    func testPrefixOperators() {
        XCTAssertEqual(.tilde, try execute(parser: OperatorParser(), source: "~").operator)
        XCTAssertEqual(.plus, try execute(parser: OperatorParser(), source: "+").operator)
        XCTAssertEqual(.minus, try execute(parser: OperatorParser(), source: "-").operator)
    }
    
    func testInfixAndPostfixOperators() {
        XCTAssertEqual(.collate("SIMPLE"), try execute(parser: OperatorParser(), source: "COLLATE SIMPLE").operator)
        XCTAssertEqual(.concat, try execute(parser: OperatorParser(), source: "||").operator)
        XCTAssertEqual(.arrow, try execute(parser: OperatorParser(), source: "->").operator)
        XCTAssertEqual(.doubleArrow, try execute(parser: OperatorParser(), source: "->>").operator)
        XCTAssertEqual(.multiply, try execute(parser: OperatorParser(), source: "*").operator)
        XCTAssertEqual(.divide, try execute(parser: OperatorParser(), source: "/").operator)
        XCTAssertEqual(.mod, try execute(parser: OperatorParser(), source: "%").operator)
        XCTAssertEqual(.plus, try execute(parser: OperatorParser(), source: "+").operator)
        XCTAssertEqual(.minus, try execute(parser: OperatorParser(), source: "-").operator)
        XCTAssertEqual(.bitwiseAnd, try execute(parser: OperatorParser(), source: "&").operator)
        XCTAssertEqual(.bitwuseOr, try execute(parser: OperatorParser(), source: "|").operator)
        XCTAssertEqual(.shl, try execute(parser: OperatorParser(), source: "<<").operator)
        XCTAssertEqual(.shr, try execute(parser: OperatorParser(), source: ">>").operator)
        XCTAssertEqual(.escape, try execute(parser: OperatorParser(), source: "ESCAPE").operator)
        XCTAssertEqual(.lt, try execute(parser: OperatorParser(), source: "<").operator)
        XCTAssertEqual(.gt, try execute(parser: OperatorParser(), source: ">").operator)
        XCTAssertEqual(.lte, try execute(parser: OperatorParser(), source: "<=").operator)
        XCTAssertEqual(.gte, try execute(parser: OperatorParser(), source: ">=").operator)
        XCTAssertEqual(.eq, try execute(parser: OperatorParser(), source: "=").operator)
        XCTAssertEqual(.eq2, try execute(parser: OperatorParser(), source: "==").operator)
        XCTAssertEqual(.neq, try execute(parser: OperatorParser(), source: "!=").operator)
        XCTAssertEqual(.neq2, try execute(parser: OperatorParser(), source: "<>").operator)
        XCTAssertEqual(.`is`, try execute(parser: OperatorParser(), source: "IS").operator)
        XCTAssertEqual(.isNot, try execute(parser: OperatorParser(), source: "IS NOT").operator)
        XCTAssertEqual(.isDistinctFrom, try execute(parser: OperatorParser(), source: "IS DISTINCT FROM").operator)
        XCTAssertEqual(.isNotDistinctFrom, try execute(parser: OperatorParser(), source: "IS NOT DISTINCT FROM").operator)
        XCTAssertEqual(.between, try execute(parser: OperatorParser(), source: "BETWEEN").operator)
        XCTAssertEqual(.and, try execute(parser: OperatorParser(), source: "AND").operator)
        XCTAssertEqual(.`in`, try execute(parser: OperatorParser(), source: "IN").operator)
        XCTAssertEqual(.match, try execute(parser: OperatorParser(), source: "MATCH").operator)
        XCTAssertEqual(.like, try execute(parser: OperatorParser(), source: "LIKE").operator)
        XCTAssertEqual(.regexp, try execute(parser: OperatorParser(), source: "REGEXP").operator)
        XCTAssertEqual(.glob, try execute(parser: OperatorParser(), source: "GLOB").operator)
        XCTAssertEqual(.isnull, try execute(parser: OperatorParser(), source: "ISNULL").operator)
        XCTAssertEqual(.notNull, try execute(parser: OperatorParser(), source: "NOT NULL").operator)
        XCTAssertEqual(.notnull, try execute(parser: OperatorParser(), source: "NOTNULL").operator)
        XCTAssertEqual(.not(.between), try execute(parser: OperatorParser(), source: "NOT BETWEEN").operator)
        XCTAssertEqual(.or, try execute(parser: OperatorParser(), source: "OR").operator)
    }
}
